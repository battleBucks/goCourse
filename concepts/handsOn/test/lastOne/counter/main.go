...
import (
        "fmt"
        "goCourse/concepts/handsOn/test/lastOne/counter/word"
)

var stringCheese = `Fourscore and seven years ago our fathers brought forth,
...
func main() {
        fmt.Println(word.Count(stringCheese))

        for k, v := range word.UseCount(stringCheese) {
                fmt.Println(v,k)
        }
